...
st.markdown("""
    <style>
    .main .block-container {import os
import datetime
import pandas as pd
import streamlit as st
    ...
    for col_name in CHILD_COLUMNS:
        ...
        if col_name in DROPDOWN_OPTIONS:
            ...
            if col_name in growth_fields_list:
                ...
                # حساب ذكي دقيق يعتمد على مقارنة الوزن والطول الحالي بالمعدل الطبيعي حسب العمر (العمر بالشهور)
                # متأخر: أقل من المعدل الطبيعي، متقدم: أكبر من المعدل الطبيعي، طبيعي: ضمن المدى المعتاد
                try:
                    ...
                    # تقدير المتوسط الطبيعي العالمي للوزن والطول حسب عمر الطفل بالشهور
                    # الوزن الطبيعي التقريبي = 3.5 كجم عند الولادة + (العمر بالشهور * 0.5 كجم)
                    # الطول الطبيعي التقريبي = 50 سم عند الولادة + (العمر بالشهور * 2.5 سم خلال أول سنة، ثم أقل بعد ذلك)
                    if age_in_months <= 12:
                    ...
                    # التحقق بناءً على طلبك: أقل من الطبيعي = متاخر، أكثر من الطبيعي = متقدم
